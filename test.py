...
def passive_dend_compartment_optimization_equivalence_test():
    print("Subtest 1: PassiveDendCompartment, v_rest=v_init=0")
    T = 16
    S = [4, 28, 28]
    TAU = 3.

    x_seq = torch.randn(size=[T, *S])
    dc = dend_compartment.PassiveDendCompartment(
        tau=TAU, decay_input=False, step_mode="s"
    )
    ...
    x_seq = torch.randn(size=[T, *S])
    dc = dend_compartment.PassiveDendCompartment(
        tau=TAU, v_rest=1.0, decay_input=False, step_mode="s"
    )
...
def main():
    ...
    elif args.mode == "optimization_equivalence":
        passive_dend_compartment_optimization_equivalence_test()
