...
def stdp_linear_single_step(
    ...
    tau_pre: float, tau_post: float, f_pre_post: Callable, f_post_pre: Callable
):
    ...
    delta_w_post_pre = (
        -f_post_pre(weight) * 
        (trace_post_syn.unsqueeze(-1) * pre_spike.unsqueeze(1)).sum(0)
    )
    delta_w_pre_post = (
        f_pre_post(weight) * 
        (trace_pre.unsqueeze(1) * post_spike_syn.unsqueeze(-1)).sum(0)
    )
...
def stdp_linear_multi_step(
    ...
    delta_w_post_pre = (
        -f_post_pre(weight) * 
        (trace_post_seq_syn.unsqueeze(-1) * pre_spike.unsqueeze(2)).sum((0, 1))
    )
    delta_w_pre_post = (
        f_pre_post(weight) * 
        (trace_pre_seq.unsqueeze(2) * post_spike_syn.unsqueeze(-1)).sum((0, 1))
    )
    return trace_pre, trace_post, delta_w_post_pre + delta_w_pre_post
...
def stdp_multi_step(f_single: Callable) -> Callable:
    def f_multi(
        ...
        f_pre_post: Callable, f_post_pre: Callable
    ):
        ...
        for t in range(T):
            trace_pre, trace_post, dw = f_single(
                ...
                tau_pre, tau_post, f_pre_post, f_post_pre
            )
            delta_w += dw
...
class STDPLearner(BaseLearner):
    ...
    def __init__(
        ...
        f_pre_post: Callable = lambda x: x, f_post_pre: Callable = lambda x: x,
        ...
        self.f_pre_post = f_pre_post
        self.f_post_pre = f_post_pre
    ...
    def step(self, on_grad: bool = True, scale: float = 1.):
        ...
        for _ in range(l):
            ...
            self.trace_pre, self.trace_post, dw = f(
                ...
                self.tau_pre, self.tau_post, self.f_pre_post, self.f_post_pre
            )
            dw *= scale
